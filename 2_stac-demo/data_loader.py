...
def load_collection(base_url, collection_id, data_dir, auth):
    """Load a STAC collection into the database."""
    collection = load_data(data_dir, "collection.json")
    collection["id"] = collection_id
    try:
        resp = requests.post(f"{base_url}/collections", json=collection, auth=auth)
...
def load_items(base_url, collection_id, use_bulk, data_dir, auth):
    ...
    load_collection(base_url, collection_id, data_dir, auth)
    if use_bulk:
        load_items_bulk_insert(
            base_url, collection_id, feature_collection, data_dir, auth
        )
    else:
...
def load_items_one_by_one(base_url, collection_id, feature_collection, data_dir, auth):
    """Load STAC items into the database one by one."""
    for feature in feature_collection["features"]:
        try:
            feature["collection"] = collection_id
            resp = requests.post(
                f"{base_url}/collections/{collection_id}/items", json=feature, auth=auth
            )
...
def load_items_bulk_insert(base_url, collection_id, feature_collection, data_dir, auth):
...
def main(base_url, collection_id, use_bulk, data_dir, username, password):
    """Load STAC items into the database."""
    auth = HTTPBasicAuth(username, password)
    load_items(base_url, collection_id, use_bulk, data_dir, auth)
